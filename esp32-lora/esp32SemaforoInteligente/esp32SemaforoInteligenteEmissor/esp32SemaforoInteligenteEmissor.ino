...
void display_init(void)
{
  ...
  {
    Serial.println("[Emissor LoRa] Falha ao inicializar comunicacao com OLED");
  }
  else
  {
    Serial.println("[Emissor LoRa] Comunicacao com OLED inicializada com sucesso");
...
void setup()
{
  ...
  display.print("Veiculo prioritario");
  display.setCursor(0, OLED_LINE2);
  display.print("ID Veiculo: ");
  display.println(identificacaoVeiculo);
  display.display();
...
void loop()
{
  ...
  identificacaoVeiculo = 2;

  /* Envia a informação */
  LoRa.beginPacket();
  LoRa.write((unsigned char *)&identificacaoVeiculo, sizeof(identificacaoVeiculo));
  LoRa.endPacket();
